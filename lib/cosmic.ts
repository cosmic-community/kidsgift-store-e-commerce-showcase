...
export async function getProductsByCollection(collectionId: string): Promise<Product[]> {
  try {
    ...
    // Filter products that include this collection in their collections array
    return allProducts.filter(product => {
      if (!product.metadata.collections) return false
      return product.metadata.collections.some(col => col.id === collectionId)
    })
  } catch (error) {
